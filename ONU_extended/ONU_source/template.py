    ...
    def action(self, cards: List[Card], last_card: Card, is_last_player_drop: bool) -> Tuple[ActionType, Card | None]: 
        ...
        def judge_drop_next(c : Card, lc : Card) -> bool:
            ...
            if isinstance(c, NumericCard) :
                ...
                if c.get_effect() == Effect.CHANGE_COLOR or c.get_effect() == Effect.PLUS_FOUR :
                    return True
        ...
        if is_last_player_drop :
            if isinstance(last_card, SpecialCard):
                match last_card.get_effect() :
                    ...
                    case Effect.PLUS_FOUR :
                        rule = lambda c, lc : isinstance(c, SpecialCard) and c.get_effect() == Effect.PLUS_FOUR
                        result = ActionType.PASS
...
class Game:
    ...
    def __init__(self, cards: List[Card], num_player: int = 7, hand_card_num: int = 7, dealer_id: int = 0) -> None:
        ...
        self._plus_cnt = 0
        self._next_player_acc = 1
        
    def get_info(self) -> Tuple[int, Card, bool, int, List[Hand]]:
        return self._current_player_id, self._last_card, self._is_last_player_drop, self._plus_cnt, self._player_hands
    
    def get_reverse(self) -> bool :
        return self._next_player_acc == -1
    
    def get_next_acc(self) -> int :
        return self._next_player_acc
    ...
    def current_player_drop_card(self, card: Card) -> None:
        ...
        if isinstance(card, SpecialCard) and card.get_effect() == Effect.PLUS_TWO :
            self._plus_cnt += 2
        if isinstance(card,SpecialCard) and card.get_effect() == Effect.REVERSE :
            self._next_player_acc *= -1
        if isinstance(card,SpecialCard) and card.get_effect() == Effect.PLUS_FOUR :
            self._plus_cnt += 4
    ...
    def turn(self) -> Tuple[Tuple[ActionType, Card | None], Tuple[int, Card, bool, int, List[Hand]], bool]:
        ...
        self._current_player_id = (self._current_player_id + self._next_player_acc) % len(self._players)
